...
public class SecurityModule {
	...
	public static void bind(final ServiceBinder binder) {
		...
		binder.bind(AuthenticationTrustResolver.class,
				...
				SpringSecurityServices.class);
		binder.bind(PasswordEncoder.class, PlaintextPasswordEncoder.class)
	...
	public static void contributeServiceOverride(
		...
		configuration.add(UsernamePasswordAuthenticationFilter.class,
				authenticationProcessingFilter);
	}

	@Marker(SpringSecurityServices.class)
	public static SaltSourceService buildSaltSource(
			@Inject @Value("${spring-security.password.salt}") final String salt)
			throws Exception {
	...
	public static void contributeComponentClassTransformWorker(
		...
		configuration.add("SpringSecurity", new SpringSecurityWorker(
				securityChecker), "after:CleanupRender");
	}
	...
	public static HttpServletRequestFilter buildFilterSecurityInterceptor(
		...
		DefaultFilterInvocationSecurityMetadataSource source = new DefaultFilterInvocationSecurityMetadataSource(
				convertCollectionToLinkedHashMap(contributions));
	...
	static LinkedHashMap<RequestMatcher, Collection<ConfigAttribute>> convertCollectionToLinkedHashMap(
		...
		for (RequestInvocationDefinition url : urls) {
			...
			requestMap.put(new AntPathRequestMatcher(url.getUrl()),
					url.getConfigAttributeDefinition());
		}
		return requestMap;
	}
	...
	public static UsernamePasswordAuthenticationFilter buildRealAuthenticationProcessingFilter(
		...
		successHandler.setAlwaysUseDefaultTargetUrl(Boolean
				.parseBoolean(alwaysUseTargetUrl));
	...
	public static HttpServletRequestFilter buildRememberMeProcessingFilter(
		...
		RememberMeAuthenticationFilter filter = new RememberMeAuthenticationFilter(authManager,rememberMe);
		filter.afterPropertiesSet();
		return new HttpServletRequestFilterWrapper(filter);
	}
	...
	public static HttpServletRequestFilter buildAnonymousProcessingFilter(
		...
		AnonymousAuthenticationFilter filter = new AnonymousAuthenticationFilter(anonymousKey);
		...
		//not sure what to pull out of attr for constructor injection
	...
	public final AuthenticationProvider buildAnonymousAuthenticationProvider(
		...
		AnonymousAuthenticationProvider provider = new AnonymousAuthenticationProvider(anonymousKey);
	...
	public final AuthenticationProvider buildRememberMeAuthenticationProvider(
			@Inject @Value("${spring-security.rememberme.key}") final String rememberMeKey)
			throws Exception {
		RememberMeAuthenticationProvider provider = new RememberMeAuthenticationProvider(rememberMeKey);
		provider.afterPropertiesSet();
		return provider;
	}
